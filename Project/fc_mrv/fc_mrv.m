function [newSolution, numCalls, space] = fc_mrv(currDomain, solution, row, column)
    ...
    % of rows. If more than one exists then we choose randomly.
    %
    % Parameters :
    %              currDomain is a matrix with all available rows for all queens
    %                 
    %              solution is the vector that shows where we had put each queen,            
    %              eg:solution=[...0...],means that in i-th queen we
    ...
    space = n^2 + 3*n; % space complexity estimation
    ...
    if (isempty(unassigned)) %we had put all the queens in rows
    ...
    for newRow = availableRows
        solution(newColumn) = newRow; % put our next queen to row and become current queen
